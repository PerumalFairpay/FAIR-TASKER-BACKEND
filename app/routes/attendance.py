...
async def get_all_attendance(
    ...
    try:
        result = await repo.get_all_attendance(
            date, start_date, end_date, employee_id, status, page, limit
        )
        return JSONResponse(
            status_code=200,
            content={
...
async def generate_attendance_records(
    ...
    try:
        ...
        result = await generate_attendance_for_date(
            date, preplanned_only=preplanned_only
        )
...
async def sync_biometric_data(
    ...
    try:
        if not payload.data:
            return JSONResponse(
                status_code=400,
                content={"message": "No data provided", "success": False},
            )
